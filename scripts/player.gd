...
@export var spriteFrames: Array[SpriteFrames]
@export var timerBufferSalto: Timer
...
var drown: int 
var saltoBuffer: bool = false;
var tiempoSaltoBuffer: float = 0.15

func _ready():
	add_to_group("personajes")
	area2D.body_entered.connect( _on_area_2d_body_entered)
	get_parent().get_parent().finNivel.connect(finDelNivel)
	$AnimatedSprite2D.sprite_frames = spriteFrames[ControladorGlobal.visibilidadSprite]
	timerBufferSalto.timeout.connect(timerBufferTimeOut)
...
func _physics_process(delta: float) -> void:	
	...
	if Input.is_action_just_pressed("ui_accept") and !is_on_floor():
		saltoBuffer = true
		timerBufferSalto.start(tiempoSaltoBuffer)
	
	if is_on_floor() and saltoBuffer == true:
		velocity.y = JUMP_VELOCITY
		saltoBuffer = false;
