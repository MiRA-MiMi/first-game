...
@export var conviniButton : TextureButton

# Called when the node enters the scene tree for the first time.
func _ready() -> void:
	if ControladorGlobal.desbloqueaPersonaje2 == true:
		self.disabled = true
	else:
...
func _pressed() -> void:
	if ControladorGlobal.monedasTotales >=10:
		conviniButton.habilitar()
		self.disabled = true
		ControladorGlobal.desbloqueaPersonaje2 = true
