...
@export var radius = Vector2.ONE * 100 # Distance on the x and y axis to orbit around the controller
@export var rotation_duration := 4.0 # How many seconds it takes for one platform to complete one rotation
@export var direction := 1 #set 1 to go clockwise and -1 to go counterclockwise
...
func _physics_process(delta):
	...
	orbit_angle_offset += direction * 2 * PI * delta / float(rotation_duration)
